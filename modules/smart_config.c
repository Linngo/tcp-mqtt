...
static ICACHE_FLASH_ATTR AP_mode(void){
	smartconfig_stop();
	WIFI_Init();
	Inter213_InitTCP(8266);
}

static ICACHE_FLASH_ATTR void timer_out(void){
	os_timer_disarm(&timer);
	os_timer_setfn(&timer, (os_timer_func_t *) AP_mode,NULL);
	os_timer_arm(&timer, 180000, false); //启动定时器，单位：毫秒
}
